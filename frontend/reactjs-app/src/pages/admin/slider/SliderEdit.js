...
const SliderEdit = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const sliderData = await Sliderervice.getById(id);

                setName(sliderData.name);
                setDescription(sliderData.desciption);
                setCreatedBy(sliderData.createdBy);
                setStatus(sliderData.status);
                setStringImageDefault(sliderData.image);
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const deleteIfExitsImage = await Sliderervice.getById(id);
            ...
            const result = await Sliderervice.update(id, sliderData);
            if (result) {
